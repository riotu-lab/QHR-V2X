...
Artifacts. No figure is committed — each is regenerated by the command shown, so
the paths below appear only after you run it:

| Artifact | Produced by |
| --- | --- |
| `benchmarks/results/benchmark_output_{dense,sparse}/csv/benchmark_results_*_selected.csv` — raw RDM / PL / RDT per algorithm per grid size | `make reproduce` |
| `experiments/results/paper_figures/Figure_{3..8}_*.png` — Figures 3–8 from this code's output | `make figures SEED=paper` |
| `experiments/results/figures/` — the same six at Table 1's parameters, 20 seeds, plus supplementary charts | `make figures-nominal` |
| `experiments/results/diagnostics/hypothesis_check_fig3.png` — provenance check behind §2.8 | `experiments/analysis/hypothesis_check.py` |
| `experiments/results/diagnostics/metric_consistency_fig3.png` — each candidate RDM definition applied uniformly to all three algorithms | `experiments/analysis/metric_consistency.py` |

`experiments/analysis/paper_figures.py` prints every value it plots, so each
point can be read off the terminal and checked against the image. The measured
CSVs under `experiments/results/` *are* committed, and carry the numbers quoted
...
`src/qhr_v2x.py:274` uses `tentative_g = g_cost[current] + 1` — unit cost per hop.
...
grids the harness actually builds (`build_grid`, `tests/test_pathfinding_all.py:49-88`):
